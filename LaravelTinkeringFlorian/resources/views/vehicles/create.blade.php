...
<head>
    ...
    <title>Creacio model</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>

<body class="bg-gray-100 p-8">
<div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6">
    <h1 class="text-3xl font-bold mb-4">Afegir model nou</h1>
    <form action="{{ route('vehicle.store') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label for="marca" class="block text-gray-700">Marca:</label>
            <input type="text" id="marca" name="marca" class="w-full px-3 py-2 border rounded" required>
        </div>
        <div class="mb-4">
            <label for="model" class="block text-gray-700">Model</label>
            <textarea id="model" name="model" class="w-full px-3 py-2 border rounded" required></textarea>
        </div>
        <div class="mb-4">
            <label for="any_fabricacio" class="block text-gray-700">Any fabricacio:</label>
            <input type="date" id="any_fabricacio" name="any_fabricacio" class="w-full px-3 py-2 border rounded" required>
        </div>
        <div class="mb-4">
            <label for="color_fabrica" class="block text-gray-700">Colors de fabrica:</label>
            <input type="text" id="color_fabrica" name="color_fabrica" class="w-full px-3 py-2 border rounded" required>
        </div>
        <div class="mb-4">
